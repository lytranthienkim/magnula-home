...
import { getAllRoomSuitabilities, createRoomSuitability, updateRoomSuitability, deleteRoomSuitability, restoreRoomSuitability } from '@/api/productAttributes';
...
export default function RoomSuitabilitiesPage() {
  ...
  const canRead = checkPermission(currentUser, PERMISSIONS.ROOM_SUITABILITIES.READ);
  const canCreate = checkPermission(currentUser, PERMISSIONS.ROOM_SUITABILITIES.CREATE);
  const canUpdate = checkPermission(currentUser, PERMISSIONS.ROOM_SUITABILITIES.UPDATE);
  const canDelete = checkPermission(currentUser, PERMISSIONS.ROOM_SUITABILITIES.DELETE);
  ...
  useEffect(() => {
    const fetchSuitabilities = async () => {
      try {
        ...
        setSuitabilities(res.data || []);
      } catch (err) {
        setError('Failed to load room suitabilities');
      } finally {
        setLoading(false);
      }
    };
  ...
  useEffect(() => {
    const filtered = (suitabilities || []).sort((a, b) => {
      const dateA = new Date(a.createdAt).getTime();
      const dateB = new Date(b.createdAt).getTime();
      return sortOrder === 'newest' ? dateB - dateA : dateA - dateB;
    });
    setDisplayData(filtered);
  }, [suitabilities, sortOrder]);

  const handleViewDetails = (suitability) => {
    setSelectedSuitability(suitability);
    setEditMode(false);
    setEditData({});
    setShowDetails(true);
  };
  ...
  const handleSave = async () => {
    ...
    try {
      await updateRoomSuitability(selectedSuitability.id, editData);
      const updated = { ...selectedSuitability, ...editData };
      setSelectedSuitability(updated);
      setSuitabilities((prev) => prev.map((s) => (s.id === selectedSuitability.id ? updated : s)));
      setEditMode(false);
      setEditData({});
    } catch (err) {
  ...
  const handleAdd = async () => {
    ...
    setFormLoading(true);
    try {
      const res = await createRoomSuitability(formData);
      setSuitabilities((prev) => [res.data, ...prev]);
  ...
  const handleRestore = async () => {
    setStatusUpdating(true);
    try {
      await restoreRoomSuitability(selectedSuitability.id);
      const updated = { ...selectedSuitability, deletedAt: null };
      setSelectedSuitability(updated);
      setSuitabilities((prev) => prev.map((s) => (s.id === selectedSuitability.id ? updated : s)));
    } catch (err) {
  ...
  const handleDeleteFromTable = async (item) => {
    ...
    setLoading(true);
    try {
      await deleteRoomSuitability(item.id);
      setSuitabilities((prev) => prev.filter((s) => s.id !== item.id));
  ...
  if (!canRead) {
    return <div className="bg-white border border-gray-200 rounded-xl p-8 text-center"><p className="text-gray-500">Access denied. You don't have permission to view room suitabilities.</p></div>;
  }
  ...
  return (
    <div>
      <div className="mb-8 flex justify-between items-center">
        <div>
          <h3 className="font-bold text-black uppercase">Room Suitabilities Management</h3>
          <p className="body-02">Manage room suitabilities</p>
        </div>
        {canCreate && (
          <button onClick={() => setShowAddForm(true)} className="flex items-center gap-2 px-4 py-2 bg-black text-white text-xs font-bold rounded hover:bg-gray-800 transition">
            <HiOutlinePlus className="w-5 h-5" /> Add Suitability
          </button>
        )}
      </div>
      ...
      <div className="flex items-center justify-between mb-6">
        <span className="text-xs text-gray-500">
          {displayData.length} of {suitabilities.length}
        </span>
      ...
      {showDetails && selectedSuitability && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-lg shadow-2xl max-w-2xl w-full max-h-[90vh] overflow-y-auto">
            <div className="px-8 py-6 border-b border-gray-200">
              <p className="text-xs text-gray-600 font-semibold uppercase mb-2">Suitability ID</p>
              <h2 className="text-3xl font-bold text-black">{selectedSuitability.name}</h2>
            </div>
            <div className="px-8 py-6 space-y-8">
              <div><p className="text-sm text-black font-semibold uppercase mb-4">Details</p><div className="space-y-4">
                <div className="flex items-start"><p className="text-xs text-black font-semibold uppercase w-32">Name:</p>{editMode ? <input type="text" value={editData.name !== undefined ? editData.name : selectedSuitability.name || ''} onChange={(e) => setEditData({ ...editData, name: e.target.value })} className="flex-1 px-4 py-2 bg-white border border-[0.5px] border-gray-400 text-xs text-black focus:outline-none" /> : <p className="flex-1 bg-gray-50 px-4 py-2 text-xs">{selectedSuitability.name || 'N/A'}</p>}</div>
                <div className="flex items-start"><p className="text-xs text-black font-semibold uppercase w-32 mt-1">Description:</p>{editMode ? <textarea value={editData.description !== undefined ? editData.description : selectedSuitability.description || ''} onChange={(e) => setEditData({ ...editData, description: e.target.value })} className="flex-1 px-4 py-2 bg-white border border-[0.5px] border-gray-400 text-xs text-black focus:outline-none" rows="3" /> : <p className="flex-1 bg-gray-50 px-4 py-2 text-xs">{selectedSuitability.description || 'N/A'}</p>}</div>
                <div className="flex items-center"><p className="text-xs text-black font-semibold uppercase w-32">Status:</p>{editMode ? <select value={editData.isActive !== undefined ? editData.isActive : selectedSuitability.isActive} onChange={(e) => setEditData({ ...editData, isActive: e.target.value === 'true' })} className="flex-1 px-4 py-2 bg-white border border-[0.5px] border-gray-400 text-xs text-black focus:outline-none"><option value="true">Active</option><option value="false">Inactive</option></select> : <span className={`flex-1 px-4 py-2 text-xs font-semibold rounded ${selectedSuitability.isActive ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-700'}`}>{selectedSuitability.isActive ? 'Active' : 'Inactive'}</span>}</div>
              </div></div>
            </div>
              ...
              {editMode ? (
                ...
                <>
                  ...
                  <div className="flex gap-2">
                    {!selectedSuitability.deletedAt ? (
                      <>
                        {canUpdate && <button onClick={() => { setEditMode(true); setEditData({ name: selectedSuitability.name, description: selectedSuitability.description, isActive: selectedSuitability.isActive }); }} className="px-6 py-2 bg-black text-white text-xs font-bold hover:bg-gray-800 transition">Edit</button>}
                      </>
                    ) : (
      ...
      {showAddForm && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-lg shadow-2xl max-w-md w-full p-8">
            <h3 className="text-xl font-bold text-black mb-6">Add Room Suitability</h3>
