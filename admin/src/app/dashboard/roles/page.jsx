...
import { getAllRoles, createRole, updateRole, restoreRole } from '@/api/roles';
import {
  RolesHeader,
  RolesTable,
  RolesModal,
...
export default function RolesPage() {
  ...
  const [formData, setFormData] = useState({ roleName: '', isActive: true });
  ...
  const handleSave = async () => {
    ...
    try {
      await updateRole(selected.id, editData);
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-lg shadow-2xl max-w-md w-full p-8">
            ...
            <div className="flex gap-3 justify-end">
              <button onClick={() => { setShowAddForm(false); setFormData({ roleName: '', isActive: true }); setError(''); }} disabled={formLoading} className="px-6 py-2 border border-gray-300 text-black text-xs font-bold rounded hover:bg-gray-50 transition disabled:opacity-50">Cancel</button>
