...
import { getAllCollectionImages, updateCollectionImage, deleteCollectionImage } from '@/api/products';
...
export default function CollectionImagesPage() {
  ...
  const handleSave = async () => {
    ...
    try {
      await updateCollectionImage(selectedImage.id, editData);
  ...
  const handleDeleteFromTable = async (item) => {
    ...
    setLoading(true);
    try {
      await deleteCollectionImage(item.id);
  ...
  const columns = [
    ...
    },
    {
      key: 'imageUrl',
      label: 'IMAGE',
      render: (row) => <img src={row.imageUrl} alt="Collection" className="h-12 w-12 object-cover rounded" />
    },
    {
      key: 'createdAt',
      label: 'CREATED',
  ...
  if (!canRead) {
    return <div className="bg-white border border-gray-200 rounded-xl p-8 text-center"><p className="text-gray-500">Access denied. You don't have permission to view collection images.</p></div>;
  }

  return (
    <div>
      <div className="mb-8">
        <h3 className="font-bold text-black uppercase">Collection Images Management</h3>
        <p className="body-02">View and manage collection images</p>
      </div>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-lg shadow-2xl max-w-2xl w-full max-h-[90vh] overflow-y-auto">
            <div className="px-8 py-6 border-b border-gray-200">
              <p className="text-xs text-gray-600 font-semibold uppercase mb-2">Collection Image ID</p>
              <h2 className="text-3xl font-bold text-black">#{selectedImage.id}</h2>
            </div>
            <div className="px-8 py-6 space-y-8">
              <div className="flex justify-center">
                <img src={selectedImage.imageUrl} alt="Collection" className="max-w-96 max-h-96 rounded object-cover" />
              </div>
              <div>
                <p className="text-sm text-black font-semibold uppercase mb-4">Collection Information</p>
                <div className="space-y-4">
                  <div className="flex items-start">
                    <p className="text-xs text-black font-semibold uppercase w-32">Collection:</p>
                    <p className="flex-1 bg-gray-50 px-4 py-2 text-xs">{selectedImage.Collection?.collectionName || 'N/A'}</p>
                  </div>
                </div>
              </div>
              <div>
                <p className="text-sm text-black font-semibold uppercase mb-4">Image Details</p>
                <div className="space-y-4">
                  <div className="flex items-start">
                    <p className="text-xs text-black font-semibold uppercase w-32">URL:</p>
                    {editMode ? (
                      <input
                        type="text"
                        value={editData.imageUrl !== undefined ? editData.imageUrl : selectedImage.imageUrl || ''}
              ...
              {editMode ? (
                ...
                <>
                  ...
                  <div className="flex gap-2">
                    {canUpdate && (
                      <button
                        onClick={() => { setEditMode(true); setEditData({ imageUrl: selectedImage.imageUrl }); }}
